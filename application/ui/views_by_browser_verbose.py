...
class views_by_browser_verbose:
  """Class for getting the verbose views by browser in the GUI
  """
  def __init__(self, tab, preprocess):
    self.views_by_browser_verbose(tab)
    self.preprocess = preprocess
    self.tab =tab
    
    
  def views_by_browser_verbose(self, tab):
    """Builds the UI for the Task 3a tab
  ...
  def build_top_browsers(self):
    """Returns the top 10 verbose browsers for display in the GUI.

    :return: top 10 verbose browsers
    :rtype: dictonary
    """
    ...
    for x in range(len(browsers)):
      top_browsers[str(browsers[x])]= self.browser_dict[browsers[x]]
    
    return top_browsers
    
  def get_browser_result_verbose(self):
    """Get the verbose browsers used to accsess documents in the selected file 
    """
    ...
    if helper_functions.is_pathname_valid(self.file_path):
      try:
        ...
        self.browser_dict = manager.views_by_browser.get_view_by_browser_verbose_dict()
        
        browsers = self.build_top_browsers()
      
        if len(browsers) != 0:
          helper_functions.build_bar_chart(browsers, self.tab, 'Verbose Browsers')
        else:
